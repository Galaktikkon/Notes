...
- problem: wysyłanie pakietu odbywa się po warstwie 2, np. adresem [[Adresacja w sieciach LAN#Adres MAC (Media Access Control)|MAC]] w [[Ethernet|Ethernecie]], a pakiet IP wysyłamy na [[Adres IPv4|adres IP]], nie znając docelowego adresu MAC
...
- działa jak tablica [[Urządzenia warstwy łącza danych#Skąd mostki wiedzą o położeniu hostów?|ForwardingDataBase]] przy [[Urządzenia warstwy łącza danych#Switch|switchach]], ale jest u hostów, a nie w urządzeniach pośrednich
...
- przesyła się całą [[Ramka|ramkę]] (np. [[Ethernet]]), w środku w polu danych siedzi ramka ARP:
...
- Pogrubiona część to **ramka ARP**, enkapsulowana w ramce Ethernet. Po kolei:
	...
	- adres źródłowy to [[Adresacja w sieciach LAN#Adres MAC (Media Access Control)|MAC]] nadawcy
	- 0806 - ramka Ethernet zawierająca ramkę ARP
	- dane - ramka ARP:
		- 0001 - używamy [[Ethernet|Ethernetu]]
		- 0800 - używamy [[Protokół IP|IP]]
...
5. Komputer A zna z ramki adres [[Adresacja w sieciach LAN#Adres MAC (Media Access Control)|MAC]] komputera B, mogą się już komunikować
# Brama (ang. *gateway*)
...
- [[Adres IPv4|adres IP]] [[Routing#Router|routera]], do którego komputery wysyłają pakiety, jeżeli ich odbiorca jest poza siecią lokalną. Jest częścią podstawowej konfiguracji hosta (więc zna ją np. od [[Protokół DHCP|serwera DHCP]], dostaje ją razem ze swoim IP).
...
3. Router tworzy odpowiedź ARP ze swoim [[Adresacja w sieciach LAN#Adres MAC (Media Access Control)|MACiem]] i odsyła do A.
...
- proxy “podstawia” swój [[Adresacja w sieciach LAN#Adres MAC (Media Access Control)|adres MAC]] za adres MAC celu (tzn. tego IP, do którego chce się wysłać pakiet), a potem sam przesyła pakiety dalej
- działanie:
...
- można włączyć bez naruszania [[Routing#Tablica routingu|tablic routingu]]
## Wady
...
- aktualizowanie tablic ARP innych hostów - przydatne np. przy backupowych serwerach, bo gdy główny padnie, to zapasowy (ten sam [[Adres IPv4|adres IP]], co główny) wyśle wszystkim ramkę GARP ze swoim [[Adresacja w sieciach LAN#Adres MAC (Media Access Control)|MACiem]], żeby wszyscy dowiedzieli się o zmianie
...
- problem: trzeba się upewnić, że ma się unikatowy adres IP, np. [[Protokół DHCP|serwer DHCP]] nie dał błędnie dwóch takich samych
- idea: coś podobnego do gratuitous ARP (też są wysyłane “nadmiarowo”), ale do innego celu
- różni się od GARP tym, że ma ustawione:
	- [[Adresacja w sieciach LAN#Adres MAC (Media Access Control)|MAC]] odbiorcy na 00:00:00:00:00:00
	- [[Adres IPv4|IP]] nadawcy na 0.0.0.0
- powyższe gwarantuje, że nic nie wykorzysta tego pakietu do update’owania tablic ARP (bo jeżeli jest konflikt adresów, to zdecydowanie nie chcemy, żeby odbiorcy się “uczyli” na tych danych), w przeciwieństwie do [[#Gratuitous ARP|GARP]], gdzie tego właśnie chcemy
